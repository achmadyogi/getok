...
for i=1:jmlfile
    ...
    disp(nama_file);
    fid=fopen(nama_file);
    %fid=fopen('82KRK_Sekutu_SBS.txt');
...
%         r=j;
    ...
    fprintf('================================================================================\n');    
    fprintf('No   Titik            xlama          ylama          Xbaru          Ybaru        \n');
    fprintf('--------------------------------------------------------------------------------\n');
        for i=1:r
            fprintf('%2d %12s %14.3f %14.3f %14.3f %14.3f %8.2f %8.2f \n',i,char(titik(i)),xlama(i),ylama(i),Xbaru(i),Ybaru(i),dx(i),dy(i));
        end
    ...
    if r>2
        ...
        % menghitung parameter transformasi 3D dg Bursa-Wolf
        [parameter,varx,V,sigma] = bursa_wolf (in_lama,in_baru);

        parameter3D_BW(1) = parameter(1);% Tx 
        parameter3D_BW(2) = parameter(2);% Ty 
        parameter3D_BW(3) = parameter(3);% Tz 
        parameter3D_BW(4) = parameter(4);% alfa
        parameter3D_BW(5) = parameter(5);% beta 
        parameter3D_BW(6) = parameter(6);% gamma
        parameter3D_BW(7) = parameter(7);% skala 
        parameter3D_BW(8) = 0.0;
        parameter3D_BW(9) = 0.0;
        parameter3D_BW(10)= 0.0;
